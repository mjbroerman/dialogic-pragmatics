...
if input("Do you want to run simulations? (y/n): ").strip().lower() == 'y':
    num_simulations = int(input("Enter the number of simulations to run (e.g., 100): "))
    
    # Ask for starting parameters or use defaults
    print("\nPlease specify the starting parameters for the simulations.")
    print("Enter 'default' to use default values, or provide specific values.")
    
    # CL's reasons-for
    cl_for_input = input("Enter 'default', a number for size, or a decimal between 0 and 1 for chance of reasons-for for CL: ").strip()
    if cl_for_input == 'default':
        cl_for_size = 'random'
        cl_for_chance = 0.5
    elif 0 < float(cl_for_input) < 1:
        cl_for_size = 'random'
        cl_for_chance = float(cl_for_input)
    else:
        cl_for_size = int(cl_for_input)
        cl_for_chance = -1  # size overrides chance
    
    # CL's reasons-against
    cl_agn_input = input("Enter 'default', a number for size, or a decimal between 0 and 1 for chance of reasons-against for CL: ").strip()
    if cl_agn_input == 'default':
        cl_agn_size = 'random'
        cl_agn_chance = 0.5
    elif 0 < float(cl_agn_input) < 1:
        cl_agn_size = 'random'
        cl_agn_chance = float(cl_agn_input)
    else:
        cl_agn_size = int(cl_agn_input)
        cl_agn_chance = -1  # size overrides chance
    
    # CR's reasons-for
    cr_for_input = input("Enter 'default', a number for size, or a decimal between 0 and 1 for chance of reasons-for for CR: ").strip()
    if cr_for_input == 'default':
        cr_for_size = 'random'
        cr_for_chance = 0.5
    elif 0 < float(cr_for_input) < 1:
        cr_for_size = 'random'
        cr_for_chance = float(cr_for_input)
    else:
        cr_for_size = int(cr_for_input)
        cr_for_chance = -1  # size overrides chance
    
    # CR's reasons-against
    cr_agn_input = input("Enter 'default', a number for size, or a decimal between 0 and 1 for chance of reasons-against for CR: ").strip()
    if cr_agn_input == 'default':
        cr_agn_size = 'random'
        cr_agn_chance = 0.5
    elif 0 < float(cr_agn_input) < 1:
        cr_agn_size = 'random'
        cr_agn_chance = float(cr_agn_input)
    else:
        cr_agn_size = int(cr_agn_input)
        cr_agn_chance = -1  # size overrides chance
    
    # Agent strategies
    print("\nThe following agent strategies are available: 'random', 'minimize AC', and 'one step ahead'.")
    cl_strat = input("Enter the strategy for CL: ").strip()
    cr_strat = input("Enter the strategy for CR: ").strip()
    
    # Initialize data storage
    data = []
    
    for i in range(num_simulations):
        seed_value = i  # Use loop index as seed for reproducibility
        ...
        # Generate inferential theories for CL and CR
        clt = RandomInferentialTheoryGenerator(
            msf=msf,
            for_move_size=cl_for_size,
            against_move_size=cl_agn_size,
            for_move_chance=cl_for_chance,
            against_move_chance=cl_agn_chance
        )
        crt = RandomInferentialTheoryGenerator(
            msf=msf,
            for_move_size=cr_for_size,
            against_move_size=cr_agn_size,
            for_move_chance=cr_for_chance,
            against_move_chance=cr_agn_chance
        )
        ...
        inq = InquiryFor(
            ...
            CR_strategy=cr_strat,
            CL_InferentialTheory=clt,
            CR_InferentialTheory=crt
        )

        # Collect data
        cl_num_for = len(clt.ForMove)
        cl_num_agn = len(clt.AgainstMove)
        cr_num_for = len(crt.ForMove)
        cr_num_agn = len(crt.AgainstMove)
        common_ground_for = len(inq.ICG.ForMove)
        common_ground_agn = len(inq.ICG.AgainstMove)
        verdict = inq.Verdict

        data.append({
            'seed': seed_value,
            'cl_num_for': cl_num_for,
            'cl_num_agn': cl_num_agn,
            'cr_num_for': cr_num_for,
            'cr_num_agn': cr_num_agn,
            'common_ground_for': common_ground_for,
            'common_ground_agn': common_ground_agn,
            'verdict': verdict
        })
    
    # Write data to CSV
    filename = input("\nEnter the filename to save the data (e.g., 'simulation_results.csv'): ").strip()
    with open(filename, 'w', newline='') as csvfile:
        fieldnames = ['seed', 'cl_num_for', 'cl_num_agn', 'cr_num_for', 'cr_num_agn', 'common_ground_for', 'common_ground_agn', 'verdict']
    ...
    print(f"\nSimulation data has been saved to {filename}")
else:
    msf = RandomMSF(language = lang)
